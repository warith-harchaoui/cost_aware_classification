...
Cost-Aware Decision-Theoretic Learning\\with Optimal Transport}}
...
\node[draw, rounded corners, align=center, minimum width=40mm, minimum height=9mm, right=of argmax] (decision) {decision\\(action)};
\draw[->] (x) -- (model);
\draw[->] (model) -- (argmax);
...
\node[draw, rounded corners, align=center, minimum width=20mm, minimum height=9mm, right=of x2] (model2) {model\\$\vp_\theta(\vx)$};
...
\node[draw, rounded corners, align=center, minimum width=40mm, minimum height=9mm, right=of risk] (dec2) {decision\\$\argmin_j r(j\mid\vp)$};
\draw[->] (x2) -- (model2);
\draw[->] (model2) -- (risk);
\draw[->] (risk) -- (dec2);
\end{tikzpicture}
...
\subsection{Fraud detection: asymmetric and delayed costs}

Fraud detection is a paradigmatic example of classification under asymmetric,
delayed, and business-critical costs.
While it is often presented as a binary classification problem
$\mathcal{Y}=\{\text{legit},\text{fraud}\}$,
real-world systems typically involve richer taxonomies
(e.g.\ suspicious, confirmed fraud, friendly fraud, chargeback-prone)
and multi-stage decision processes.

\paragraph{From prediction to operational decision.}
In practice, a model prediction does not merely label a transaction:
it triggers an operational action such as \emph{allow}, \emph{block},
\emph{step-up authentication}, or \emph{manual review}.
These actions have immediate and delayed consequences that extend beyond
the correctness of the prediction itself.

\paragraph{Step 1: articulate business value.}
The starting point is not the model, but the business objective.
For each pair $(y,a)$ of ground truth outcome $y$ and operational action $a$,
one should articulate a value (or utility) $V(y,a)$,
expressed in a common unit (e.g.\ monetary value, risk points, or expected profit).
This value aggregates multiple effects:
\begin{itemize}
\item direct financial impact (e.g.\ chargebacks, refunds),
\item operational costs (e.g.\ manual review effort),
\item customer experience and churn,
\item reputational or regulatory risk.
\end{itemize}

\begin{table}[h]
\centering
\caption{Illustrative value matrix $\mV$ for a binary fraud detection task.}
\begin{tabular}{c|cc}
\toprule
Reality $\backslash$ Action & Allow & Block \\
\midrule
Legitimate & $+10$ & $-5$ \\
Fraudulent & $-50$ & $0$ \\
\bottomrule
...
Learning objectives are more naturally expressed in terms of costs.
For each realized outcome $y$, let
\[
a^\star(y) \in \argmax_{a} V(y,a)
\]
denote the optimal action under perfect information.
The regret (or cost) associated with action $a$ is then defined as
\[
C_{y,a} \;=\; V(y,a^\star(y)) - V(y,a).
\]
By construction, this yields a cost matrix $\mC$ such that:
\begin{itemize}
\item costs are non-negative,
\item $C_{y,a^\star(y)} = 0$ for all $y$,
\item costs are comparable across actions and classes.
\end{itemize}

\begin{table}[h]
\centering
\caption{Corresponding cost (regret) matrix $\mC$.}
\begin{tabular}{c|cc}
\toprule
Reality $\backslash$ Action & Allow & Block \\
\midrule
...
\paragraph{Step 3: expected cost under uncertainty.}
At prediction time, the true class $Y$ is unknown.
Given a predictive distribution $\vp$,
the expected cost of choosing action $a$ is
\[
r(a \mid \vp) = \sum_{y \in \mathcal{Y}} p_y \, C_{y,a}.
\]
The rational decision is the Bayes-optimal action
\[
\hat a(\vp) = \argmin_a r(a \mid \vp),
\]
which depends on the full predictive distribution,
not merely on the most likely class.

\paragraph{Asymmetry and delay.}
Two characteristics make fraud detection particularly challenging:
\begin{itemize}
\item \textbf{Asymmetry:} false negatives (missed frauds) typically dominate the cost structure,
while false positives primarily affect customer experience and long-term value.
\item \textbf{Delay and uncertainty:} the true outcome may only be observed weeks later
(e.g.\ via chargebacks), and sometimes imperfectly.
Costs therefore represent \emph{expected} regrets over a stochastic business process,
rather than immediate losses.
\end{itemize}

\paragraph{Implications for learning.}
Training a model with cross-entropy implicitly assumes
that all errors are equally undesirable and instantaneous.
In contrast, a cost matrix $\mC$ makes the operational trade-offs explicit
and induces a non-trivial geometry on the label space.
Cost-aware objectives---and in particular Optimal Transport losses---allow the learning process
to internalize these trade-offs directly, rather than correcting for them post hoc
via thresholds or rules.

\paragraph{Extension to multi-class and multi-stage fraud.}
In richer fraud taxonomies, costs can reflect escalation pathways:
confusing a legitimate transaction with a low-risk anomaly
may be far less costly than confusing it with confirmed fraud.
Similarly, confusion between adjacent stages in an investigation pipeline
may incur smaller regret than skipping stages altogether.
Such structures are naturally encoded by a cost matrix
and exploited by OT-based learning objectives.
...
\subsection{From Sinkhorn divergence to Sinkhorn distance}
...
This section defines the \emph{CACIS} loss as a Fenchel--Young (FY) loss induced by
a Sinkhorn-based (OT-based) regularizer. The goal is to train a classifier whose
probabilistic outputs are intrinsically shaped by a task-specific misclassification
geometry, rather than applying costs only \emph{post hoc} at decision time.
...
$Y\in\{1,\dots,K\}$, and $\mC\in\R_+^{K\times K}$ is a cost matrix such that
$C_{yy}=0$.
At prediction time, the model outputs a score vector (logits) $f_\theta(\vx)\in\R^K$
(often $f_\theta(\vx)=\vz_\theta(\vx)$), from which we derive a distribution over labels.

\subsection{CACIS as a Fenchel--Young loss}

Let $\Omega:\Delta^K\to\R$ be a strictly convex regularizer on the simplex.
The Fenchel--Young loss generated by $\Omega$ is
\begin{equation}
\ell_{\Omega}(y,f)
\;=\;
\Omega^*(f) - f_y + \Omega(\ve_y),
\label{eq:fy_loss}
\end{equation}
...
\bigl(\langle \valpha,f\rangle - \Omega(\valpha)\bigr).
...
\label{eq:q_def}
\end{equation}
and the loss gradient w.r.t.\ scores is the familiar ``prediction error'' form:
\begin{equation}
...
\label{eq:fy_grad}
\end{equation}
Thus, once $q(f)$ is available, CACIS backpropagates like cross-entropy,
but with a \emph{cost-structured} mapping $f\mapsto q(f)$.

\subsection{Sinkhorn negentropy regularizer}

CACIS specializes~\eqref{eq:fy_loss} by choosing a cost-aware regularizer built from
entropically-regularized OT. Fix a cost matrix $\mC$ and $\varepsilon>0$.
Define the \emph{Sinkhorn negentropy} regularizer
\begin{equation}
...
\end{equation}
where $\OT_{\mC}^{\varepsilon}$ is the entropic OT cost defined in~\eqref{eq:entropic_ot}.
The resulting FY loss
\begin{equation}
\ell_{\CACIS}(y,f;\mC,\varepsilon)
\;\coloneqq\;
\ell_{\Omega_{\mC,\varepsilon}}(y,f)
\end{equation}
is the \emph{CACIS loss}.

\paragraph{Variational form of the conjugate.}
Following the geometric-loss derivations for OT-regularized FY losses,
one obtains an explicit variational form for the conjugate:
\begin{equation}
\Omega_{\mC,\varepsilon}^*(f)
=
-\varepsilon \log\!\left(
\min_{\valpha\in\Delta^K}
\valpha^\top \mM(f,\mC,\varepsilon)\,\valpha
\right),
\label{eq:omega_star_variational}
\end{equation}
where $\mM(f,\mC,\varepsilon)\in\R^{K\times K}$ has entries
\begin{equation}
M_{ij}
\;=\;
\exp\!\left(
...
\label{eq:M_entries}
\end{equation}
Intuitively, $\mM$ mixes \emph{scores} and \emph{pairwise costs} into a smooth kernel
that biases probability mass toward low-cost confusions.

\subsection{Computing the loss: Frank--Wolfe on the simplex}

To evaluate~\eqref{eq:omega_star_variational} and obtain $q(f)=\nabla \Omega^*(f)$,
...
\end{equation}
where $\mM=\mM(f,\mC,\varepsilon)$.
We use Frank--Wolfe (conditional gradients) because it is simple, stable on the simplex,
and has a cheap linear oracle.
...
In implementation, we treat the optimizer output $\valpha^\star$ as defining the FY mapping
$f\mapsto q(f)$ (cf.\ \eqref{eq:q_def}) and backpropagate through the outer network,
without differentiating through the Frank--Wolfe iterates themselves.
This ``implicit'' use of the argmin is numerically stable and typically sufficient in deep learning pipelines.

\paragraph{Interpretation.}
Cross-entropy corresponds to a particular (isotropic) choice of $\Omega$ (Shannon negentropy),
while CACIS replaces it by the OT/Sinkhorn-induced regularizer~\eqref{eq:sinkhorn_negentropy},
thereby injecting the label geometry encoded by $\mC$ directly into the learning objective.
